...
import TopReactedPosts, { type TopPostsTab } from "@/components/TopReactedPosts";
...
export default async function Home() {
  ...
  let topPostsByTab: Record<TopPostsTab, Post[]> = {
    all: [],
    sighting: [],
    chat: [],
    meme: [],
  ...
  if (supabase) {
    ...
    const fetchTop = (category?: (typeof CATEGORIES)[number]) => {
      let query = supabase
        .from("posts")
        .select(POST_COLUMNS)
        .eq("status", "published")
    ...
    const [{ data: allTop }, { data: sightingTop }, { data: chatTop }, { data: memeTop }] =
      await Promise.all([
        fetchTop(),
        fetchTop("sighting"),
        fetchTop("chat"),
  ...
  return (
    <div className="mx-auto max-w-6xl px-5 py-12">
      <div className="lg:grid lg:grid-cols-[1fr_minmax(0,42rem)_1fr] lg:items-start lg:gap-6">
        ...
        <div className="mx-auto w-full max-w-2xl">
          <section className="sticker-card relative overflow-hidden bg-canvas-mint p-8 text-center sm:p-12">
            ...
            <p className="relative mx-auto mt-3 max-w-md text-sm text-ink-soft sm:text-base">
              로그인 없이, 이름 없이 &mdash; 오늘도 지나가는 모브 하나로
              목격담과 잡담을 살짝 흘려놓고 가는 곳이에요.
            </p>
          </section>
          ...
          <section className="mt-12 lg:hidden">
            ...
            <div className="mt-4">
              <TopReactedPosts initialPostsByTab={topPostsByTab} configured={configured} />
            </div>
          </section>
          ...
          <section className="mt-12">
            ...
            <div className="mt-4">
              <RecentPosts initialPosts={recentPosts} configured={configured} />
            </div>
          </section>
        </div>
